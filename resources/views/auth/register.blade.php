...
<head>
    ...
    <style>
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .card-sub {
            font-size: .875rem; color: #94a3b8; margin-bottom: 28px;
        }
        ...
        .form-group .toggle-pw:hover { color: #94a3b8; }
        ...
        .star { animation: twinkle var(--d,3s) var(--delay,0s) infinite ease-in-out; }

        @keyframes dashScroll {
            from { stroke-dashoffset: 0; }
            to   { stroke-dashoffset: -40; }
        }
        .road-dash { animation: dashScroll 1s linear infinite; }
        ...
        .car-group { animation: carGlide 4s ease-in-out infinite; }

        @keyframes beamPulse { 0%,100%{opacity:.55} 50%{opacity:.85} }
        .beam { animation: beamPulse 2s ease-in-out infinite; }

        @keyframes winBlink { 0%,95%,100%{opacity:1} 97%{opacity:.1} }
        ...
        .alert { animation: slideDown .25s ease; }
...
<svg class="bg-scene" viewBox="0 0 1200 700" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
    ...
    <!-- Sky -->
    <rect width="1200" height="700" fill="url(#rsky)"/>

    <!-- Moon -->
    <circle cx="1060" cy="90" r="60" fill="url(#rmoonGlow)"/>
    ...
    <rect x="75"  y="218" width="4"   height="462" fill="url(#rbldGlow)"/>
    ...
    <g fill="#93c5fd" opacity=".55">
        ...
        <rect x="104" y="334" width="13" height="9" rx="1"/>
    </g>
    ...
    <rect x="480" y="148" width="4"   height="552" fill="url(#rbldGlow)"/>
    ...
    <rect x="0" y="568" width="1200" height="80"  fill="url(#rroad)"/>
    ...
    <!-- Headlight beams -->
    <polygon class="beam" points="140,604 80,568 80,628" fill="url(#rcone)" opacity=".35"/>
    <polygon class="beam" points="140,616 65,590 65,644"  fill="url(#rcone)" opacity=".25"/>
...
<main class="page-center">
    <div class="card">
        ...
        <p class="card-sub">Daftar dan nikmati layanan rental terbaik</p>

        <!-- ── SUCCESS ALERT (tampil setelah register berhasil, via Laravel session) ── -->
        {{-- @if(session('success')) --}}
        <div class="alert alert-success" id="alertSuccess" style="display:none">
            ...
            <div>
                <strong>Akun berhasil didaftarkan!</strong><br>
                <span style="font-size:.8rem;opacity:.85">Silakan masuk menggunakan email dan password Anda.</span>
            </div>
        </div>
        {{-- @endif --}}

        <!-- ── ERROR ALERT (tampil jika ada error umum, mis. email sudah terdaftar) ── -->
        {{-- @if($errors->any()) --}}
        <div class="alert alert-error" id="alertError" style="display:none">
            ...
            <span id="alertErrorMsg">Terjadi kesalahan. Periksa kembali data Anda.</span>
        </div>
        {{-- @endif --}}
        ...
        <form id="registerForm" method="POST" action="{{ route('register') }}" novalidate>
            ...
            <div class="grid-2">
                ...
                <div class="form-group col-full">
                    ...
                    {{-- @error('name') --}}
                    <p class="field-error @error('name') visible @enderror" id="err-name">
                ...
                <div class="form-group">
                    ...
                    <span class="field-icon toggle-pw" onclick="togglePw('password','iconPw1')">
                        <svg id="iconPw1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                             fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="form-group">
                    ...
                    <span class="field-icon toggle-pw" onclick="togglePw('password_confirmation','iconPw2')">
                        <svg id="iconPw2" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                             fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>
                        </svg>
                    </span>
                    <p class="field-error" id="err-confirm">Password tidak cocok</p>
                </div>
...
function checkStrength(val) {
    ...
    const map = ['','weak','weak','medium','strong'];
    const labels = ['','Lemah','Lemah','Sedang','Kuat'];
    for (let i = 0; i < score; i++) bars[i].className = 'pw-bar ' + map[score];
    label.textContent = labels[score];
    label.style.color = score <= 2 ? '#ef4444' : score === 3 ? '#f59e0b' : '#10b981';
}
...
document.getElementById('registerForm').addEventListener('submit', function(e) {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) { setErr('email', true); valid = false; } else setErr('email', false);
    if (pw.value.length < 8) { setErr('password', true); valid = false; } else setErr('password', false);

    const noMatch = pw.value !== pwc.value;
    document.getElementById('err-confirm').classList.toggle('visible', noMatch);
    pwc.classList.toggle('is-error', noMatch);
    if (noMatch) valid = false;
...
/* ══ Demo: show alerts on load for preview ══ */
/* Remove the block below in production — alerts are controlled by Laravel session/errors */
(function() {
    ...
    if (params.get('status') === 'error') {
        ...
        document.getElementById('alertErrorMsg').textContent =
            params.get('msg') || 'Email sudah terdaftar. Gunakan email lain atau masuk langsung.';

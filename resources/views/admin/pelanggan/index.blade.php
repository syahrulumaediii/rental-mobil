...
<div class="card p-4 mb-5">
    <form method="GET" class="flex flex-col sm:flex-row flex-wrap gap-3 items-end">
        <div class="flex-1 w-full sm:min-w-44">
        ...
        <div class="w-full sm:w-auto">
            ...
            <select name="status_verifikasi" class="form-input w-full sm:w-48">
                <option value="">Semua</option>
                {{-- Diubah dari 'approved' menjadi 'verified' agar sinkron dengan database --}}
                @foreach(['belum_verifikasi'=>'Belum Verifikasi','pending'=>'Pending','verified'=>'Verified','rejected'=>'Rejected'] as $val=>$lbl)
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="hidden sm:table-cell">Kota</th>
                    <th>Verifikasi</th>
                    <th class="hidden lg:table-cell">Blacklist</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td class="hidden md:table-cell"><span class="font-mono text-sm">{{ $p->nik ?? '-' }}</span></td>
                    <td class="hidden sm:table-cell">{{ $p->kota ?? '-' }}</td>
                    ...
                    <td class="text-right">
                        {{-- Tombol Mata ini yang mengarah ke file detail (show.blade.php), tempat admin memproses tombol terima/tolak dokumen --}}
...
{{-- Memastikan tombol halaman hanya muncul jika controller memanggil fungsi ->paginate() --}}

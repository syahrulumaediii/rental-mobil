...
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 space-y-5">
        <div class="card p-6 text-center">
            <div class="w-16 h-16 bg-primary-100 rounded-full flex items-center justify-center font-extrabold text-primary-700 text-2xl mx-auto mb-3">
            ...
            <div class="mt-3">
                ...
                @if($pelanggan->isBlacklisted()) <span class="badge badge-red ml-1"><i data-lucide="ban" class="w-3 h-3 mr-1"></i>Blacklist</span> @endif
            </div>
        </div>
        ...
        <div class="card p-5">
            ...
            <div class="flex justify-between py-1.5 border-b border-slate-50 last:border-0 text-sm">
                <span class="text-slate-400">{{ $lbl }}</span>
                <span class="font-medium text-slate-700">{{ $val }}</span>
            </div>
            @endforeach
            @if($pelanggan->alamat)
            <div class="mt-2 text-sm"><p class="text-slate-400 mb-1">Alamat</p><p class="text-slate-700">{{ $pelanggan->alamat }}</p></div>
            @endif
        </div>
        ...
        <div class="card">
            ...
            <div class="divide-y divide-slate-50">
                ...
                <div class="px-5 py-4 flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('admin.dokumen.verifikasi', $dok) }}" class="flex gap-2">
                        @csrf @method('PATCH')
                        <input type="hidden" name="status" value="verified">
                        <button class="px-3 py-1.5 bg-green-50 text-green-700 text-xs font-semibold rounded-lg hover:bg-green-100">Verified</button>
                    </form>
                    <button onclick="openRejectDok({{ $dok->id }})" class="px-3 py-1.5 bg-red-50 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-100">Tolak</button>
                    @endif
                </div>
                @empty
...
<div id="rejectDokModal" class="hidden fixed inset-0 bg-black/50 z-50 items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-sm p-6 shadow-xl">
        ...
        <form id="rejectDokForm" method="POST">
            @csrf @method('PATCH')
            <input type="hidden" name="status" value="ditolak">
            <div class="mb-4">
                <label class="form-label">Catatan (opsional)</label>
                <textarea name="catatan" rows="2" class="form-input" placeholder="Alasan penolakan..."></textarea>
            </div>
...
@push('scripts')
<script>
function openRejectDok(id) {
    document.getElementById('rejectDokForm').action = `/admin/dokumen/${id}/verifikasi`;

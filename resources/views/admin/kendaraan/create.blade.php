...
<div class="max-w-3xl mx-auto px-1 sm:px-0">
    <div class="card p-4 sm:p-6">
        <form method="POST"
            ...
            {{-- Grid dikonfigurasi 1 kolom di HP dan 2 kolom mulai dari ukuran Medium (Tablet/PC) --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">

                <div class="md:col-span-2">
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Nama Kendaraan</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Merk</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Model</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Tahun</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Plat Nomor</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Warna</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Kapasitas (Orang)</label>
                    <input type="number" name="kapasitas" value="{{ old('kapasitas', $kendaraan->kapasitas ?? '') }}" class="form-input w-full text-sm" required min="1">
                </div>

                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Transmisi</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Bahan Bakar</label>
                ...
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Tarif Harian (Rp)</label>
                    <input type="number" name="tarif_harian" value="{{ old('tarif_harian', $kendaraan->tarif_harian ?? '') }}" class="form-input w-full text-sm font-semibold" required min="0" step="1000" placeholder="300000">
                    @error('tarif_harian')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                {{-- 🌟 BARU: Input Nilai Denda Keterlambatan Per Jam --}}
                <div>
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Denda Keterlambatan Per Jam (Rp)</label>
                    <input type="number" name="denda_per_jam" value="{{ old('denda_per_jam', $kendaraan->denda_per_jam ?? '') }}" class="form-input w-full text-sm font-semibold border-amber-300 focus:border-amber-500 focus:ring-amber-500 bg-amber-50/20" required min="0" step="1000" placeholder="25000">
                ...
                <div class="md:col-span-2">
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Foto Kendaraan</label>
                    ...
                    <div class="mb-3">
                        ...
                        <p class="text-[11px] text-slate-400 mt-1.5">Upload baru untuk mengganti foto unit</p>
                    </div>
                    @endif
                    <input type="file" name="foto" accept="image/*" class="form-input w-full text-sm bg-slate-50/50 file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    @error('foto')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div class="md:col-span-2">
                    <label class="form-label mb-1.5 block text-xs font-semibold text-slate-500 uppercase">Deskripsi</label>
                    <textarea name="deskripsi" rows="3" class="form-input w-full text-sm" placeholder="Deskripsi singkat tentang kondisi atau fasilitas tambahan kendaraan...">{{ old('deskripsi', $kendaraan->deskripsi ?? '') }}</textarea>
                </div>
            </div>

            {{-- Tombol aksi adaptif: bertumpuk di HP, mendatar beriringan di PC --}}

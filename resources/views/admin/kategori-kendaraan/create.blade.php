...
@section('breadcrumb', 'Admin / Create Kategori Kendaraan')
@section('sidebar-nav')
...
<div class="container mx-auto px-4 py-6 max-w-lg">
    <div class="bg-white rounded-lg shadow-md p-6">
        <h1 class="text-xl font-bold text-gray-800 mb-6">Tambah Kategori Kendaraan</h1>

        <form action="{{ route('admin.kategori-kendaraan.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" 
                    class="shadow appearance-none border @error('deskripsi') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
